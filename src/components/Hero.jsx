...
function ParticleSkull() {
    ...
    void main() {
        ...
        float effectRange = 0.3;  // Adjust this to control repulsion radius
        float repulsionStrength = 0.5;  // Adjust to control repulsion intensity
        
        float dist = distance(screenPosition, mousePos);
        if (dist < effectRange) {
            // Calculate repulsion vector (opposite of direction)
            ...
            repulsionFactor *= repulsionFactor;  // Quadratic falloff
...
`,
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            // Calculate vibration intensity based on scroll position
...
export default function Hero() {
    ...
    const handleDownload = (url = "src/assets/TAN_Resume.pdf") => {
        fetch(url)
          ...
          .then((blob) => {
            ...
            link.href = url;
            link.download = "TanmayNawlakhe_resume.pdf" || "downloaded-file";
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
            trigger: ".gsp-element", // Ensure this matches your class name
            start: "top 90%", // Adjust this to control when the animation starts
            end: "top 0%", // Optional: you can define an end point if needed
            toggleActions: "play reverse play reverse", // Change to prevent reverse
            markers: false,
        }
    });
    ...
    return (
        <div style={{ width: '100vw', height:'115vh', paddingTop:'8vh' }} className='px-24  relative bg-[#1e1c1c]  '>
            <Canvas camera={{ position: [0, 0, 5], fov: 50 }} style={{ width: '100%', height: '105vh' }}>
            ...
            <div data-hoverable='true' className='top-[15%] flex flex-col justify-between h-[100vh]  absolute'>
                <div className='absolute top-0 left-0 transition-all duration-700 ease-in-out bg-gradient-to-tr from-yellow-300 to-blue-900 via-red-500 bg-clip-text text-transparent font-thin text-7xl gsp-element'>
                    TANMAY A NAWLAKHE
                </div>
                <div onClick={()=>handleDownload()} className='absolute cursor-pointer left-0 border-slate-400 border-[1px] py-2 px-4 text-white bg-gradient-to-l bottom-52 gsp-element2'>Resume</div>
            </div>
        </div>
    );
}